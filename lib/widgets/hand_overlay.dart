...
/// the camera preview. Coordinates are normalized (0..1) in display space.
class HandOverlayPainter extends CustomPainter {
  ...
  Offset _p(HandLandmark lm, Size size) =>
  ...
  void _drawGuideLines(Canvas canvas, Size size) {
    // Air-piano press line.
    final double? pressY = output.pressLineY;
    if (pressY != null) {
      final Paint line = Paint()
        ..color = AppTheme.primary.withValues(alpha: 0.8)
        ..strokeWidth = 2;
      final double y = pressY * size.height;
      canvas.drawLine(Offset(0, y), Offset(size.width, y), line);
      _label(canvas, 'press below', Offset(8, y + 4),
  ...
  void _drawCursors(Canvas canvas, Size size) {
    for (final FingerCursor c in output.cursors) {
      ...
      final Offset p = Offset(c.x * size.width, c.y * size.height);
